...
var (
	...
	requirePrivacyForGlobalMapping = flag.Bool("require-privacy-for-global-mapping", true, "pass -RequirePrivacy $true to New-SmbGlobalMapping on Windows node; set to false to stop requiring SMB encryption on the global mapping (only applies when --enable-windows-host-process=true and --global-mapping-additional-params is empty)")
	globalMappingAdditionalParams  = flag.String("global-mapping-additional-params", "", "additional parameters appended to New-SmbGlobalMapping on Windows host process nodes, e.g. '-RequirePrivacy $false -RequireIntegrity $true'; takes precedence over --require-privacy-for-global-mapping")
...
func handle() {
	driverOptions := smb.DriverOptions{
		...
		GlobalMappingAdditionalParams:  *globalMappingAdditionalParams,
