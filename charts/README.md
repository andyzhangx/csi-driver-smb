...
| `windows.globalMappingAdditionalParams`                 | comma-separated structured `key=value` options for `New-SmbGlobalMapping`; only applies when `useHostProcessContainers` is true and takes precedence over `windows.requirePrivacyForGlobalMapping` | `""`                                                   |
