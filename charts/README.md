...
| `windows.requirePrivacyForGlobalMapping`                | require SMB encryption (`-RequirePrivacy`) on `New-SmbGlobalMapping`; only applies when `useHostProcessContainers` is true and `windows.globalMappingAdditionalParams` is empty | `true`                                                  |
| `windows.globalMappingAdditionalParams`                 | additional raw PowerShell parameters appended to `New-SmbGlobalMapping`; only applies when `useHostProcessContainers` is true and takes precedence over `windows.requirePrivacyForGlobalMapping` | `""`                                                   |
